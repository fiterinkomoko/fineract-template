...
Feature: Create Loan Product and It's charges Steps
  ...
  @ignore
  @createLoanProductWithOverdueChargeAndCanAccommodateLargeMoneyAndSchedulesSteps
  Scenario: Create loan product With Overdue Charge that can support big schedule
    Given configure ssl = true
    * def productsData = read('classpath:templates/loanProduct.json')
    Given path 'loanproducts'
    And header Accept = 'application/json'
    And header Authorization = authToken
    And header fineract-platform-tenantid = tenantId
    And request productsData.loanProductWithOverDueChargeAndCanAccommodateLargeSchedulesAndMoneyPayload
    When method POST
    Then status 200
    Then match $ contains { resourceId: '#notnull' }
    Then def loanProductId = response.resourceId
