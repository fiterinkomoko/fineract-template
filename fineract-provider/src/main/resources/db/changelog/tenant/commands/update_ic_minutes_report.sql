...
UPDATE stretchy_report SET report_sql = "select c.external_id as 'Client UID', concat(c.firstname,c.lastname) as 'Client/Company Name', l.loan_counter as 'Cycle', cvg.code_value as 'Gender', cvs.code_value as  'Strata',
...
icReviewTbl.IC_Level_Five as 'IC Decision Level Five', l.approved_principal as 'IC Approved Amount', l.term_frequency as 'Loan Tenure in months', '' as 'IC Approved Amount in Words', l.principal_disbursed_derived as 'Installment Amount',
...
and (date(l.submittedon_date) between date('${startDate}') and date('${endDate}'))  " WHERE id = (SELECT tbl.id FROM (SELECT sr.id FROM stretchy_report sr WHERE sr.report_name = "IC Minutes") AS tbl);
