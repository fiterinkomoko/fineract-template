...
UPDATE stretchy_report SET report_sql = "select concat(u.firstname, ' ', u.lastname) as 'User', concat(c.firstname, ' ', c.lastname) as 'Client Name', c.external_id as 'Client UUID', l.account_no as 'Loan Number', loanPurposeTble.code_value as 'Purpose',
...
left join (select * from m_loan_transaction lt group by lt.id, lt.loan_id, lt.submitted_on_date order by lt.submitted_on_date desc limit 1 )  lastPaymentTbl on lastPaymentTbl.loan_id = l.id
...
from m_loan_repayment_schedule lrs where lrs.completed_derived = false group by lrs.loan_id, lrs.duedate, lrs.principal_amount,
lrs.principal_completed_derived, lrs.principal_writtenoff_derived, lrs.interest_amount, lrs.interest_completed_derived,
lrs.interest_waived_derived, lrs.interest_writtenoff_derived,lrs.fee_charges_amount,lrs.fee_charges_completed_derived,
lrs.fee_charges_waived_derived,lrs.fee_charges_writtenoff_derived) nextPaymentTbl  on nextPaymentTbl.loan_id = l.id
where o.id = ${officeId}
