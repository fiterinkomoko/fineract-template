...
UPDATE stretchy_report SET report_sql = "select DISTINCT l.account_no                                                  as 'Loan Number',
                                                  ...
                                                  from m_office o
                                                           ...
                                                           left join (SELECT MIN(DATEDIFF(NOW(), lrs.duedate))                                                        AS minDays,
                                                                             MAX(lrs.duedate)                                                                         AS nextPaymentDueDate,
                                                                             lrs.loan_id,
                                                                             SUM(IFNULL(lrs.principal_amount, 0) - IFNULL(lrs.principal_completed_derived, 0) -
                                                                                 IFNULL(lrs.principal_writtenoff_derived, 0))                                         AS scheduledPrincipalAmount,
                                                                             SUM(IFNULL(lrs.interest_amount, 0) - IFNULL(lrs.interest_completed_derived, 0) -
                                                                                 IFNULL(lrs.interest_waived_derived, 0) -
                                                                                 IFNULL(lrs.interest_writtenoff_derived, 0))                                          AS scheduledInterestAmount,
                                                                             SUM(IFNULL(lrs.fee_charges_amount, 0) - IFNULL(lrs.fee_charges_completed_derived, 0) -
                                                                                 IFNULL(lrs.fee_charges_waived_derived, 0) -
                                                                                 IFNULL(lrs.fee_charges_writtenoff_derived, 0))                                       AS scheduledFeesAmount
                                                                      FROM m_loan_repayment_schedule lrs
                                                                      WHERE lrs.completed_derived = false
                                                                      GROUP BY lrs.loan_id
                                                  ) nextPaymentTbl on nextPaymentTbl.loan_id = l.id
                                                  where o.id = ${officeId}
