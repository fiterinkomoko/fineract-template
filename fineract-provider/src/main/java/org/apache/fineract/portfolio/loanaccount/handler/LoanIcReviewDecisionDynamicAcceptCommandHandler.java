...
/**
 * Command handler for dynamic IC Review Decision Accept.
 * This handler processes IC review decisions for levels 6 and above.
 * The level number is extracted from the command URL.
 */
@Slf4j
@Service
@RequiredArgsConstructor
@CommandType(entity = "LOANICREVIEWDECISIONDYNAMIC", action = "ACCEPT")
public class LoanIcReviewDecisionDynamicAcceptCommandHandler implements NewCommandSourceHandler {

    private final LoanApplicationDecisionWritePlatformService loanApplicationDecisionWritePlatformService;

    // Pattern to extract level number from href like /loans/decision/icReviewDecision/level/{levelNumber}/{loanId}
    ...
    public CommandProcessingResult processCommand(final JsonCommand command) {
        ...
        log.debug("Processing dynamic IC review accept for level {} on loan {}", levelNumber, command.getLoanId());
        return this.loanApplicationDecisionWritePlatformService.acceptIcReviewDecisionDynamic(
                command.getLoanId(), command, levelNumber);
    }

    /**
     * Extracts the level number from the command URL.
     * Expected URL pattern: /loans/decision/icReviewDecision/level/{levelNumber}/{loanId}
     */
