...
public interface RefBankRepository extends JpaRepository<RefBank, Long> {
    ...
    @Query("SELECT b FROM RefBank b WHERE b.isActive = true AND (" +
           ...
           "LOWER(b.country) LIKE LOWER(CONCAT('%', :query, '%')) OR " +
           "LOWER(b.bankCode) LIKE LOWER(CONCAT('%', :query, '%')))" +
           "ORDER BY b.country ASC, b.bankName ASC")
    List<RefBank> searchBanks(@Param("query") String query);
}
