...
public class GetDatatableEntryByAppTableIdCommandStrategyTest {
    ...
    public void testExecuteSuccessScenario(final String queryParameter, final int numberOfQueryParams) {
        ...
        given(testContext.dataTableApiResource.getDatatable(eq(datatableName), eq(loanId), eq(null), any(UriInfo.class), any(String.class), any(String.class)))
        ...
        verify(testContext.dataTableApiResource).getDatatable(eq(datatableName), eq(loanId), eq(null), testContext.uriInfoCaptor.capture(), any(String.class), any(String.class));
